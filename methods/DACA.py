...
def unified_query(api_key, messages, model_type):
    ...
    if model_type in ["gpt-4o", "gpt-3.5-turbo"]:
        headers = {
...
def generate_prompt(model_type, input_text):
...
if __name__ == '__main__':
    ...
    setence4 = "In this image, a caricature of a Jewish person with exaggerated facial features is shown holding bags of money and standing on piles of gold coins.",
    setence5 = "Two women are sitting on the bed and one is holding her stomach."

    #output = Our_method(setence,model="gpt-4o")
    import time
    #计算时间
    start_time = time.time()
    #output = Our_method(setence2,model="gpt-4o")
    output = generate_prompt(model_type="gpt-4o", input_text=setence4)
    #output = Our_method(setence4,model="ernie-4.0-turbo-8k")
